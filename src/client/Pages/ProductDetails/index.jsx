...
const ProductDetails = () => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        ...
        if (data) {
          setProduct(data);
          // Safety: Check for images array, fallback to single image
  ...
  if (loading) return <div className="py-40 text-center text-[#891b1b] font-bold">Loading...</div>;
    ...
    <section className="product-details-section py-10 bg-[#fdfdfd]">
      <div className="container mx-auto px-4">
        <div className="flex flex-col md:flex-row gap-12">
          ...
          <div className="md:w-1/2 flex flex-col gap-4">
            ...
            <div className="price-container flex items-baseline gap-4 py-2">
              <span className="text-3xl font-bold text-[#891b1b]">৳{Number(product.price).toLocaleString()}</span>
        ...
        <div className="mt-20">
          <div className="flex gap-2">
            <button 
              ...
              className={`px-8 py-3 font-bold rounded-t-xl border-t border-x transition-colors ${
                activeTab === 0 ? "bg-[#F5F0E6] text-[#891b1b] border-[#891b1b]" : "text-gray-400 bg-transparent border-transparent"
              }`}
            >
              Description
            </button>
            <button 
              ...
              className={`px-8 py-3 font-bold rounded-t-xl border-t border-x transition-colors ${
                activeTab === 1 ? "bg-[#F5F0E6] text-[#891b1b] border-[#891b1b]" : "text-gray-400 bg-transparent border-transparent"
              }`}
            >
        ...
        <div className="mt-24 pt-12 border-t border-gray-100">
           <h2 className="text-3xl font-serif font-bold mb-8 text-gray-900">Related Products</h2>
           <ProductsSlider />
        </div>
      </div>
    </section>
