...
const CartBar = () => {
  ...
  // Ensure cartItems is always treated as an array to prevent crashes
  const safeCartItems = Array.isArray(cartItems) ? cartItems : [];

  const handleClose = () => setIsCartOpen?.(false);
  
  // Calculate total price safely with defensive checks
  ...
  /**
   * REFINED IMAGE LOGIC
   * Safely handles deep property access
   */
    ...
    <Drawer anchor="right" open={!!isCartOpen} onClose={handleClose}>
      <div className="w-[350px] sm:w-[400px] h-full flex flex-col bg-white">
        ...
        <div className="p-4 flex items-center justify-between border-b">
          <h2 className="text-lg font-bold">Shopping Cart ({safeCartItems.length})</h2>
        ...
        <div className="flex-1 overflow-y-auto p-4">
          {safeCartItems.length === 0 ? (
            ...
            safeCartItems.map((item, index) => {
              ...
              return (
                <div key={`${pId || 'item'}-${item?.size || index}-${index}`} className="flex gap-4 py-4 border-b hover:bg-gray-50 transition-colors">
