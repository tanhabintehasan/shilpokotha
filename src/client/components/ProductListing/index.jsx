...
const ProductListing = () => {
  ...
  const { addToCart, onAddToWishlist } = useShop(); 

  const productsPerPage = 8;
  const BACKEND_URL = "http://localhost:5000";

  // Data Fetching Logic - NO FILTERING
  const fetchProducts = useCallback(async () => {
    try {
      setLoading(true);
      const res = await axios.get(`${BACKEND_URL}/api/products`);
      
      // Get all data regardless of designType or Category
      const allData = Array.isArray(res.data) ? res.data : res.data.products || [];
      
      setProducts(allData); // Set everything to state
      setCurrentPage(1); 
    } catch (err) {
  ...
  const currentItems = sortedProducts.slice(indexOfFirstProduct, indexOfLastProduct);
  ...
  if (loading) return (
    <div className="h-screen flex items-center justify-center bg-[#d1c9bd]">
      <div className="text-center font-bold text-[#891b1b]">Loading All Items...</div>
    </div>
  );
      ...
      <div className="container mx-auto flex flex-col lg:flex-row gap-6 items-start">
        ...
        <div className="rightcontent flex-1 w-full">
          ...
          <div className="productHeader flex flex-col md:flex-row gap-4 items-center justify-between bg-white/80 backdrop-blur-md p-5 rounded-3xl mb-8 shadow-sm border border-white/40">
            <div className="flex items-center gap-6">
              ...
              <span className="text-sm font-bold text-gray-600">
                Displaying <span className="text-[#891b1b]">{products.length}</span> Total Items
              </span>
            </div>

            <div className="flex items-center gap-3">
              <span className="text-[10px] font-black uppercase text-gray-400">Sort By</span>
              <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="bg-white border rounded-xl px-4 py-2 text-xs font-bold outline-none">
          ...
          {currentItems.length > 0 ? (
            ...
            <div className="bg-white/50 rounded-[2.5rem] p-24 text-center">
               <h3 className="text-2xl font-black text-gray-400">No items found in database.</h3>
            </div>
          )}
          ...
          {products.length > productsPerPage && (
            <div className="mt-16 flex justify-center">
              <Pagination count={Math.ceil(products.length / productsPerPage)} page={currentPage} onChange={handlePageChange} />
            </div>
          )}
        </div>
      </div>
